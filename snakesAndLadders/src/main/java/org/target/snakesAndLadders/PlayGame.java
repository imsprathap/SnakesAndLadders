...
/**
 * This class is used to play the game, all the logic is present in the class
 * @author sprathap
 *
 */
public class PlayGame {
	...
	private static final String NEXT_POS_STRING = "Your new position is : ";
		...
		for (PlayerModel playerModel : playerList) {
			...
			if (diceValue != 0) {
				...
				if (updatedPosition < gameModel.getnBlocks()) {
					PlayerModel updatedPlayerModel = findNextPosition(playerModel, gameModel, scanner, updatedPosition,
							0);
	...
	private void handleHealth(PlayerModel playerModel, GameModel gameModel) {
		...
		if (!(health > 0)) {
			System.out.println(
					Constants.ANSI_RED + "Sorry, Your health is Zero. Need to start from 1" + Constants.ANSI_RESET);
	...
	private PlayerModel findNextPosition(PlayerModel playerModel, GameModel gameModel, Scanner scanner,
			int updatedPosition, int recurssionValue) {
		try {
			...
			if (null != property && !Constants.EMPTY.equalsIgnoreCase(property)) {
				if (property.equalsIgnoreCase(Constants.LADDER)) {
					...
					System.out.println(NEXT_POS_STRING + playerModel.getCurrentPos());
				...
				if (null != property && !Constants.EMPTY.equalsIgnoreCase(property)
						&& !Constants.PITSTOP.equalsIgnoreCase(property) && updatedPosition <= gameModel.getnBlocks()) {
	...
	public boolean userInputToPlay(Scanner scanner, String message) {
		...
		while (true) {
			...
			if (null != decision && !"".equalsIgnoreCase(decision)
					&& (decision.equalsIgnoreCase("Y") || decision.equalsIgnoreCase("N"))) {
